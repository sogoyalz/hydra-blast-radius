...
Plus two evals and a test suite: a [round-trip correctness gate](#correctness-eval) (independently-computed ground truth vs. HydraDB's own traversal, cross-referenced against real OSV advisories), [validation against deps.dev](#validation-against-an-independent-source) so the *data* is graded by an outside resolver and not only by this project's own crawl, and [51 unit tests](#unit-tests) (`npm test`) covering the union arithmetic, id collisions, escaping, timeouts, semver ordering, advisory ranges and the typosquat filters. Served through a zero-dependency API + browser visualization (`node src/server.js`, then `http://127.0.0.1:8787`).
