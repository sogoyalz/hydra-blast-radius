...
`SKIP`/`LIMIT` are supported and reach past the cap: `SKIP 1024` returns real rows, so the limit is on rows returned per request rather than rows the engine will consider. Every query whose result can outgrow 1024 — the blast-radius traversal, the shared-maintainer traversal (one row per maintainer-package pair, so it saturates soonest), and the package list behind the typosquat scan — now pages through `runQueryPaged()` in `src/hydra.js`. The same 1,500-package target returns **1500** through all three paths. Nothing else in this repo needed changing, because nothing else can exceed a thousand rows.
